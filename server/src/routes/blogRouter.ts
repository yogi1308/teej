...
blogRouter.post(
    "/",
    upload.fields([{ name: "file" }, { name: "cover-art" }]),
    async (req, res, next) => {
        try {
            const stage = process.env.NODE_ENV === "production" ? "prod" : "dev";
            if (!req.files?.["cover-art"] && !req.files?.["file"])
                return res
                    .status(400)
                    .json({ success: false, error: "Cover or file required" });
            const [thumbnailUpload, blogUpload] = await Promise.all([
                req.files?.["cover-art"]?.[0]
                    ? uploadImageToCloudinary(
                        req.files["cover-art"][0].buffer,
                        `${stage}/blog`,
                    )
            ...
            const created = await blogFileUploadQuery(
                req,
                thumbnailUpload,
                blogUpload,
            );
