...
import { Overlay } from "../overlay/UploadingOverlay";
...
export default function EditDialog({
    ...
    const [blogUploadtype, setBlogUploadType] = useState<"upload" | "type">(
        item.type === "blog" && item.contentType === "html" ? "type" : "upload",
    );
    ...
    useEffect(() => {
        ...
        setEditorContent("");
        setFormChange(false);
        if (item.type === "blog" && item.contentType === "html" && item.contentUrl) {
            fetch(item.contentUrl)
                .then(res => res.text())
                .then(html => setEditorContent(html))
                .catch(() => { });
            setBlogUploadType("type");
        } else if (item.type === "blog") {
            setBlogUploadType("upload");
        }
    }, [item]);
    ...
    async function handleUpload() {
        ...
        try {
            ...
            fd.set("imgsChange", String(imgsChange));
            fd.set("songsChange", String(songsChange));
            fd.set("blogChange", String(blogChange));
            fd.set("blogUploadtype", blogUploadtype);

            if (imgsChange) {
                fd.set("imgsUrl", JSON.stringify(imgsUrl));
                fd.set("imgsAssetIds", JSON.stringify(item.imageAssetId ?? []));
            }
    ...
    return (
        ...
        >
            <dialog
                className="dialog flex-col bg-black/50 overflow-hidden p-4 pb-0 backdrop-blur-md max-w-[80vw]! gap-4 min-h-40 fixed inset-0 m-auto border border-white max-h-[90vh]"
                ref={dialogRef}
            >
                                    ...
                                    .map(([key, value]) => (
                                        <div className="flex-1">
                                            <AddInput
                                                ...
                                                defaultValue={value ?? ""}
                                                placeholder={value ?? ""}
                                                type={key === "releaseDate" ? "date" : typeof value === "number" ? "number" : "text"}
                                            />
                                        </div>
                                    ))}
                            </div>
                                    ...
                                    {blogUploadtype === "upload" && (
                                        <div className="flex flex-col gap-1 ">
                                            ...
                                            <input
                                                ...
                                                onChange={e => {
                                                    if (e.target.files?.[0]) {
                                                        setToast("Blog file has been changed");
                                                        setBlogChange(true);
                                                    }
                                                }}
                                            />
